...
QImage  cvMatToQImage( const cv::Mat &inMat )
{
   switch ( inMat.type() )
   {
      ...
      default:
         throw std::runtime_error("ASM::cvMatToQImage() - cv::Mat image type not handled in switch: " + std::to_string(inMat.type()));
         break;
   }
...
cv::Mat QImageToCvMat( const QImage &inImage, bool inCloneImageData )
{
   switch ( inImage.format() )
   {
      ...
      {
         if ( !inCloneImageData )
            throw std::runtime_error("ASM::QImageToCvMat() - Conversion requires cloning since we use a temporary QImage");
      ...
      default:
         throw std::runtime_error("ASM::QImageToCvMat() - QImage format not handled in switch: " + std::to_string(inImage.format()));
         break;
   }
